    ...
    private function transliterate_term($term, $search_mode)
    {
        if ($search_mode == 'auto') {
            if (Serbian_Transliteration_Utilities::is_cyr($term)) {
                return $this->cyr_to_lat($term);
            } else {
                return $this->lat_to_cyr($term, false, (get_rstr_option('fix-diacritics', 'no') != 'no'));
            }
        } else if ($search_mode == 'plugin-mode') {
			
			if(get_rstr_option('site-script', 'lat') == 'cyr') {
				return $this->lat_to_cyr($term, false, get_rstr_option('fix-diacritics', 'no') == 'yes');
			} else {
				return $this->cyr_to_lat($term);
			}
        }
