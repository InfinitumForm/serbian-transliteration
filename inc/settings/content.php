...
class Serbian_Transliteration_Settings_Content extends Serbian_Transliteration
{
	...
	function __construct($object)
	{
		...
		$this->add_action('serbian_transliteration_settings_content', 'nav_tab_wrapper');
		$this->add_action('serbian_transliteration_settings_content', 'tab_content');
		
		$this->add_action('serbian_transliteration_settings_tab', 'nav_tab_settings');
		$this->add_action('serbian_transliteration_settings_tab', 'nav_tab_permalink_tool');
		
		if(is_null($this->tab))
		{
			$this->add_action('serbian_transliteration_settings_tab_content', 'settings_form');
			$this->add_action('serbian_transliteration_settings_tab_content', 'available_shortcodes');
		}
		else
		{
			$this->add_action('serbian_transliteration_settings_tab_content__settings', 'settings_form');
			$this->add_action('serbian_transliteration_settings_tab_content__settings', 'available_shortcodes');
			
			$this->add_action('serbian_transliteration_settings_tab_content__permalink_tool', 'tab_content_permalink_tool');
		}
...
<div class="tab-content" id="tab-<?php $this->tab ? printf('%s-%s', RSTR_NAME, $this->tab) : RSTR_NAME; ?>">
	<?php 
		do_action('serbian_transliteration_settings_before_tab_content', $this);
		if(is_null($this->tab)) {
			do_action('serbian_transliteration_settings_tab_content', $this);
		} else {
			do_action('serbian_transliteration_settings_tab_content__' . $this->tab, $this->tab);
		}
		do_action('serbian_transliteration_settings_after_tab_content', $this);
	?>
</div>
	<?php }
...
<nav class="nav-tab-wrapper">
	<?php do_action('serbian_transliteration_settings_tab', $this);?>
</nav>
	<?php }
	...
	**/
	public function ajax__run_permalink_transliteration () {
		...
		{
			// Posts per page
			$posts_pre_page = apply_filters('serbian_transliteration_tool_permalink_transliteration_offset', 20);
